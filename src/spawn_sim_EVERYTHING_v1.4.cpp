// spawn_sim_neon_ptrstep_eor3_dualpartial_rowstep_inplace_adults.cpp
//
// Builds on spawn_sim_neon_ptrstep_eor3_dualpartial_rowstep.cpp with one
// additional state-layout optimization:
//
//   inplace_adults: A(t+1) is written directly into the current_juveniles
//                   buffer instead of a separate next_adults buffer.
//                   This removes one full-grid write stream from the
//                   steady-state generation update.
//
//                   The recurrence A(t+1) = J(t) | surviving_adults means
//                   the juvenile buffer is only needed once per row before
//                   it can be safely overwritten.  The hot interior overwrite
//                   is safe because the edge/wrapping fallback paths only
//                   read old juvenile bits from border words that the hot
//                   kernel never touches.
//
//                   Edge-word paths now use a masked replace rather than
//                   OR-into-prefilled, so wrap columns in current_juveniles
//                   that the wrapping pass still needs are preserved.
//
//                   Generation rotation changes from 5 buffers to 4:
...
//
// inplace_adults: out_next_adults now points into the juvenile buffer.
// The old juvenile value is read before any write occurs here.
// ---------------------------------------------------------------------------
    ...
    uint64x2_t in_b0,  uint64x2_t in_b1,  uint64x2_t in_b2)
{
    ...
    // inplace_adults: write A(t+1) into out_next_adults which points into
    // the juvenile buffer.  juvenile_word was already loaded above.
...
[[maybe_unused]] static int count_adult_neighbors_naive(
...
//
// inplace_adults: row0_next_adults / row1_next_adults now point into the
// juvenile buffer.  The old juvenile values are read inside emit_and_slide_neon
// before any write occurs at those locations.
// ---------------------------------------------------------------------------
...
__attribute__((noinline)) static void run_two_hot_rows_pair2_neon_ptrstep(
    ...
    uint64_t* __restrict row0_next_adults,   // points into current_juveniles
    uint64_t* __restrict row0_next_eggs,
    uint64_t* __restrict row1_next_adults,   // points into current_juveniles
...
//
// inplace_adults: next_adults_ptr now points into current_juveniles.data().
// The pointer is used both as a read source (old juvenile values) and as a
// write destination (new adult values).  Each word is read before it is
// written within the same iteration.
// ---------------------------------------------------------------------------
...
__attribute__((noinline)) static void run_one_generation_hot_rows(
    ...
    BitPlane& current_juveniles,       // read old J, write A(t+1) in place
    ...
    HotRowScratch& hot_row_scratch)
{
    ...
    uint64_t* __restrict next_adults_ptr    = current_juveniles.data(); // inplace
    ...
    for (; row_index + 2 < hot_row_end; row_index += 2) {
        ...
        if (use_neon) {
            // -- Scalar preamble (word 1) ------------------------------------
            {
                ...
                // inplace_adults: juveniles[fi0] is read here, written below
            ...
            if (word_index < aligned_neon_end) {
                ...
                run_two_hot_rows_pair2_neon_ptrstep(
                    ...
                    next_adults_ptr + afi0,   // inplace: points into juveniles
                    next_eggs_ptr   + afi0,
                    next_adults_ptr + afi1,   // inplace: points into juveniles
                    next_eggs_ptr   + afi1,
        ...
        if (use_neon) {
            ...
            for (; word_index < aligned_neon_end; word_index += neon_words) {
                ...
                store_hot_result_from_total_with_center_u64_neon(
                    ...
                    vld1q_u64(eggs      + afi),
                    next_adult, next_egg);
                // inplace_adults: write A(t+1) into juvenile buffer
...
//
// inplace_adults: next_adults no longer exists.  Only next_eggs is cleared.
// Border rows/words that were previously prefilled into next_adults are now
// handled by the edge paths writing directly into current_juveniles.
// ---------------------------------------------------------------------------
    ...
    BitPlane& next_eggs)
{
    ...
    for (int row_index = row_begin; row_index < row_end; ++row_index) {
        ...
        if (row_index < hot_row_begin || row_index >= hot_row_end) {
            ...
            next_eggs_ptr[lw] = 0ULL;
            next_eggs_ptr[rw] = 0ULL;
        }
    }
}
...
__attribute__((always_inline)) static inline HorizontalPartialWord
compute_horizontal_partial_left_edge_valid_scalar(
    const uint64_t* __restrict adult_row)
{
    const uint64_t curr = adult_row[0];
    const uint64_t next = adult_row[1];
    const uint64_t left_2  = curr << 2;
    const uint64_t left_1  = curr << 1;
    const uint64_t right_1 = (curr >> 1) | (next << 63);
    const uint64_t right_2 = (curr >> 2) | (next << 62);
...
__attribute__((always_inline)) static inline HorizontalPartialWord
compute_horizontal_partial_right_edge_valid_scalar(
    const uint64_t* __restrict adult_row, int last_word_offset)
{
    const uint64_t prev = adult_row[last_word_offset - 1];
    const uint64_t curr = adult_row[last_word_offset];
    const uint64_t left_2  = (curr << 2) | (prev >> 62);
    const uint64_t left_1  = (curr << 1) | (prev >> 63);
    const uint64_t right_1 = curr >> 1;
    const uint64_t right_2 = curr >> 2;
...
__attribute__((always_inline)) static inline HorizontalPartialWord
compute_horizontal_partial_left_edge_wrapping_scalar(
    const uint64_t* __restrict adult_row, int last_word_offset)
{
    const uint64_t prev = adult_row[last_word_offset];
    const uint64_t curr = adult_row[0];
    const uint64_t next = adult_row[1];
    const uint64_t left_2  = (curr << 2) | (prev >> 62);
    const uint64_t left_1  = (curr << 1) | (prev >> 63);
...
__attribute__((always_inline)) static inline HorizontalPartialWord
compute_horizontal_partial_right_edge_wrapping_scalar(
    const uint64_t* __restrict adult_row, int last_word_offset)
{
    const uint64_t prev = adult_row[last_word_offset - 1];
    const uint64_t curr = adult_row[last_word_offset];
    const uint64_t next = adult_row[0];
    const uint64_t left_2  = (curr << 2) | (prev >> 62);
    const uint64_t left_1  = (curr << 1) | (prev >> 63);
    const uint64_t right_1 = (curr >> 1) | (next << 63);
    const uint64_t right_2 = (curr >> 2) | (next << 62);
...
__attribute__((always_inline)) static inline VerticalTotalWord
build_total_from_five_partials(
    HorizontalPartialWord p0, HorizontalPartialWord p1, HorizontalPartialWord p2,
    HorizontalPartialWord p3, HorizontalPartialWord p4)
{
    VerticalTotalWord total;
    add_horizontal_partial_to_total(
        p0.b0, p0.b1, p0.b2, total.b0, total.b1, total.b2, total.b3, total.b4);
    add_horizontal_partial_to_total(
        p1.b0, p1.b1, p1.b2, total.b0, total.b1, total.b2, total.b3, total.b4);
    add_horizontal_partial_to_total(
        p2.b0, p2.b1, p2.b2, total.b0, total.b1, total.b2, total.b3, total.b4);
    add_horizontal_partial_to_total(
        p3.b0, p3.b1, p3.b2, total.b0, total.b1, total.b2, total.b3, total.b4);
    add_horizontal_partial_to_total(
        p4.b0, p4.b1, p4.b2, total.b0, total.b1, total.b2, total.b3, total.b4);
    return total;
}

// ---------------------------------------------------------------------------
// store_masked_result_from_total
//
// inplace_adults: destination is current_juveniles, which still holds old
// juvenile bits in the invalid columns that the wrapping pass will read later
// in the same generation.  Use masked replace instead of OR-into-prefilled
// so those still-needed columns are preserved.
// ---------------------------------------------------------------------------

__attribute__((always_inline)) static inline void store_masked_result_from_total(
    const VerticalTotalWord& total,
    uint64_t adult_word, uint64_t juvenile_word, uint64_t egg_word,
    uint64_t valid_mask,
    uint64_t& next_adult_word, uint64_t& next_egg_word)
{
    uint64_t adult_out = 0ULL;
    uint64_t egg_out   = 0ULL;
    store_hot_result_from_total_with_center(
        total.b0, total.b1, total.b2, total.b3, total.b4,
        adult_word, juvenile_word, egg_word,
        adult_out, egg_out);
    // Masked replace: preserve invalid columns, write only valid ones.
    next_adult_word = (next_adult_word & ~valid_mask) | (adult_out & valid_mask);
    next_egg_word   |= egg_out & valid_mask;
}

// ---------------------------------------------------------------------------
// Edge / wrapping row passes — all retargeted to write adults into
// current_juveniles (next_adults no longer exists).
// ---------------------------------------------------------------------------
...
__attribute__((noinline)) static void run_one_generation_left_edge_valid_rows(
    ...
    const BitPlane& current_eggs,
    BitPlane& juveniles_as_next_adults,   // inplace: same as current_juveniles
    BitPlane& next_eggs)
{
    const uint64_t* __restrict adults      = current_adults.data();
    const uint64_t* __restrict juveniles   = current_juveniles.data();
    const uint64_t* __restrict eggs        = current_eggs.data();
    uint64_t* __restrict next_adults_ptr   = juveniles_as_next_adults.data();
    uint64_t* __restrict next_eggs_ptr     = next_eggs.data();
    ...
    for (int row_index = row_begin; row_index < row_end; ++row_index) {
        ...
        const VerticalTotalWord total = build_total_from_five_partials(
            compute_horizontal_partial_left_edge_valid_scalar(adults + rm2),
            compute_horizontal_partial_left_edge_valid_scalar(adults + rm1),
            compute_horizontal_partial_left_edge_valid_scalar(adults + r0),
            compute_horizontal_partial_left_edge_valid_scalar(adults + rp1),
            compute_horizontal_partial_left_edge_valid_scalar(adults + rp2));

        store_masked_result_from_total(
            total,
            adults[r0], juveniles[r0], eggs[r0], valid_mask,
            next_adults_ptr[r0], next_eggs_ptr[r0]);
    }
}
    ...
    BitPlane& next_eggs)
{
    ...
    for (int row_index = row_begin; row_index < row_end; ++row_index) {
        ...
        const VerticalTotalWord total = build_total_from_five_partials(
            compute_horizontal_partial_right_edge_valid_scalar(adults + rm2, last_word_offset),
            compute_horizontal_partial_right_edge_valid_scalar(adults + rm1, last_word_offset),
            compute_horizontal_partial_right_edge_valid_scalar(adults + r0,  last_word_offset),
            compute_horizontal_partial_right_edge_valid_scalar(adults + rp1, last_word_offset),
            compute_horizontal_partial_right_edge_valid_scalar(adults + rp2, last_word_offset));

        store_masked_result_from_total(
            total,
            adults[edge_wi], juveniles[edge_wi], eggs[edge_wi], valid_mask,
            next_adults_ptr[edge_wi], next_eggs_ptr[edge_wi]);
    }
}
    ...
    BitPlane& next_eggs)
{
    ...
    const int last_word_offset      = words_per_row - 1;
    const uint64_t left_wrap_mask   = 3ULL;
    const uint64_t right_wrap_mask  = 3ULL << 62;

    auto process_wrapping_word = [&](int row_index, int word_index, uint64_t valid_mask) {
        const int row_m2 = wrap_coordinate(row_index - 2, grid_size);
        const int row_m1 = wrap_coordinate(row_index - 1, grid_size);
        const int row_p1 = wrap_coordinate(row_index + 1, grid_size);
        const int row_p2 = wrap_coordinate(row_index + 2, grid_size);

        const uint64_t* __restrict row_rm2 =
            adults + static_cast<size_t>(row_m2) * words_per_row;
        const uint64_t* __restrict row_rm1 =
            adults + static_cast<size_t>(row_m1) * words_per_row;
        const uint64_t* __restrict row_r0 =
            adults + static_cast<size_t>(row_index) * words_per_row;
        const uint64_t* __restrict row_rp1 =
            adults + static_cast<size_t>(row_p1) * words_per_row;
        const uint64_t* __restrict row_rp2 =
            adults + static_cast<size_t>(row_p2) * words_per_row;

        auto partial_for = [&](const uint64_t* __restrict row_ptr) {
            if (word_index == 0)
                return compute_horizontal_partial_left_edge_wrapping_scalar(
                    row_ptr, last_word_offset);
            if (word_index == last_word_offset)
                return compute_horizontal_partial_right_edge_wrapping_scalar(
                    row_ptr, last_word_offset);
            return compute_horizontal_partial_scalar_from_adult_row(row_ptr, word_index);
        };
        ...
        const size_t flat_word_index =
            static_cast<size_t>(row_index) * words_per_row + word_index;
        store_masked_result_from_total(
            total,
            adults[flat_word_index], juveniles[flat_word_index], eggs[flat_word_index],
            valid_mask,
            next_adults_ptr[flat_word_index], next_eggs_ptr[flat_word_index]);
    };
    ...
    for (int row_index = row_begin;
        ...
        for (int word_index = 0; word_index < words_per_row; ++word_index)
            process_wrapping_word(row_index, word_index, ~0ULL);
    }
    ...
    for (int row_index = interior_begin; row_index < interior_end; ++row_index) {
        process_wrapping_word(row_index, 0, left_wrap_mask);
        process_wrapping_word(row_index, last_word_offset, right_wrap_mask);
    }
    ...
    BitPlane& next_eggs)
{
    run_one_generation_left_edge_valid_rows(
        ...
        juveniles_as_next_adults, next_eggs);
...
//
// inplace_adults: next_adults parameter removed; current_juveniles is passed
// through to all sub-passes as both source and destination for adult bits.
// ---------------------------------------------------------------------------
    ...
    HotRowScratch& hot_row_scratch)
{
    ...
    run_one_generation_hot_rows(
        row_begin, row_end, grid_size, words_per_row,
        current_adults, current_juveniles, current_eggs,
        next_eggs, hot_row_scratch);
    run_one_generation_edge_rows(
        row_begin, row_end, grid_size, words_per_row,
        current_adults, current_juveniles, current_eggs,
        current_juveniles, next_eggs);
}
...
int main(int argc, char* argv[])
{
    ...
    // inplace_adults: next_adults removed; 4 buffers instead of 5.
    ...
    // inplace_adults: 4-buffer rotation.
