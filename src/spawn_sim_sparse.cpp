...
// rolling-update hot loop with fused incoming-partial production and an
// exact two-row blocked hot update for the Monster Spawning Grid reference
// simulator.
//
// INPUT FORMAT:
...
struct alignas(64) HorizontalPartialPair2 {
    uint64_t b0[2] = {0ULL, 0ULL};
    uint64_t b1[2] = {0ULL, 0ULL};
    uint64_t b2[2] = {0ULL, 0ULL};
};

static_assert(sizeof(HorizontalPartialPair2) == 64);
static_assert(alignof(HorizontalPartialPair2) == 64);
...
inline void subtract_horizontal_partial_from_total_u64_sve(
    ...
    svuint64_t& total_b4)
{
    // BIC(a,b) = a & ~b: replaces EOR(all_ones)+AND at b0 borrow and b3 borrow
    // svnot_x: replaces EOR(all_ones) for not_b1/not_b2, frees all_ones register
    // BIC for AND arg of BSL: independent of not_b1/not_b2, shorter critical path
    // sveor3: fuses two-EOR chain for b1 and b2 total updates
    svuint64_t borrow = svbic_x(pg, partial_b0, total_b0);    // p0 & ~b0

    total_b0 = sveor_x(pg, total_b0, partial_b0);

    const svuint64_t not_b1 = svnot_x(pg, total_b1);          // ~b1 (no all_ones reg)
    svuint64_t next_borrow = svbsl_u64(
        svorr_x(pg, not_b1, partial_b1),
        svbic_x(pg, partial_b1, total_b1),                     // p1 & ~b1 (indep of not_b1)
        borrow);
    total_b1 = sveor3(total_b1, partial_b1, borrow);           // b1^p1^borrow
    borrow = next_borrow;

    const svuint64_t not_b2 = svnot_x(pg, total_b2);
    next_borrow = svbsl_u64(
        svorr_x(pg, not_b2, partial_b2),
        svbic_x(pg, partial_b2, total_b2),                     // p2 & ~b2
        borrow);
    total_b2 = sveor3(total_b2, partial_b2, borrow);
    borrow = next_borrow;

    next_borrow = svbic_x(pg, borrow, total_b3);               // borrow & ~b3
    total_b3 = sveor_x(pg, total_b3, borrow);
    total_b4 = sveor_x(pg, total_b4, next_borrow);
}


// Fast path: when rolling total ≤ 7 for all cells (b3=b4=0).
// Simplifies adult_5_to_10 → b2 & (b1|b0)  and  egg_3_to_5 → egg_inner.
// Valid iff caller has confirmed svorr(vb3,vb4) == 0 for all SVE lanes.
__attribute__((always_inline)) static inline void
store_hot_result_fast_le7(
    svbool_t pg,
    svuint64_t vb0, svuint64_t vb1, svuint64_t vb2,
...
{
    // adult survives if 5 ≤ total ≤ 7  →  b2 & (b1|b0)
    // egg spawns if 3 ≤ total ≤ 5      →  (~b2 & b1&b0) | (b2 & ~b1)  (= egg_inner)
    const svuint64_t b1_or_b0  = svorr_x(pg, vb1, vb0);
    const svuint64_t adult_cond = svand_x(pg, vb2, b1_or_b0);
    const svuint64_t egg_cond  = svorr_x(pg,
        svbic_x(pg, svand_x(pg, vb1, vb0), vb2),   // ~b2 & b1&b0
        svbic_x(pg, vb2, vb1));                      // b2 & ~b1
    const svuint64_t blocked = svorr_x(pg, juvenile_word, egg_word);
    const svuint64_t empty   = svnot_x(pg, svorr_x(pg, adult_word, blocked));
    next_adult_word = svorr_x(pg, svand_x(pg, adult_word, adult_cond), juvenile_word);
    next_egg_word   = svand_x(pg, empty, egg_cond);
}
...
inline void store_hot_result_from_total_with_center_u64_sve(
    ...
    svuint64_t& next_egg_word)
{
    // BIC(a,b)=a&~b eliminates 5 explicit EOR-with-all_ones NOTs:
    //   not_b4, not_b3, not_b2, not_b1, ~b1_and_b0  (saves 6 ops vs 25 old)
    ...
    const svuint64_t empty_word    = svnot_x(pg, occupied_word);
    ...
    const svuint64_t A = svand_x(pg,
        svbic_x(pg, total_b2, total_b3),        // b2 & ~b3
        b1_or_b0);
    const svuint64_t B = svbic_x(pg,
        svbic_x(pg, total_b3, total_b2),        // b3 & ~b2
        b1_and_b0);                              //    & ~(b1&b0)
    const svuint64_t adult_5_to_10 =
        svbic_x(pg, svorr_x(pg, A, B), total_b4); // ~b4 & (A|B)

    // egg_3_to_5 = ~b4 & ~b3 & ((~b2&(b1&b0)) | (b2&~b1))
    const svuint64_t C = svbic_x(pg, b1_and_b0, total_b2); // ~b2 & b1_and_b0
    const svuint64_t D = svbic_x(pg, total_b2,  total_b1); // b2  & ~b1
    const svuint64_t egg_3_to_5 =
        svbic_x(pg,
            svbic_x(pg, svorr_x(pg, C, D), total_b3),
            total_b4);
    ...
    HotRowScratch& hot_row_scratch)
{
    ...
    for (; row_index + 2 < hot_row_end; row_index += 2) {
        ...
        if (use_aligned_main_hot_loop) {
            ...
            if (row_index + 4 < hot_row_end) {
                const size_t pf2 = static_cast<size_t>(row_index + 2) * words_per_row;
                const size_t pf3 = static_cast<size_t>(row_index + 3) * words_per_row;
                __builtin_prefetch(juveniles + pf2, 0, 0);
                __builtin_prefetch(eggs      + pf2, 0, 0);
                __builtin_prefetch(juveniles + pf3, 0, 0);
                __builtin_prefetch(eggs      + pf3, 0, 0);
            }
            for (; word_index < aligned_simd_word_end; word_index += sve_words) {
                ...
                // HOIST: row1 DRAM loads (adults/juvs/eggs) before L2 partial-pair work
                // Gives ~80 cycles of L2/compute to cover DRAM latency for row1 data
                const svuint64_t h_adult1 = svld1_u64(pg, adults    + aligned_flat_word_index1);
                const svuint64_t h_juv1   = svld1_u64(pg, juveniles + aligned_flat_word_index1);
                const svuint64_t h_egg1   = svld1_u64(pg, eggs      + aligned_flat_word_index1);
                ...
                svuint64_t next_adult_word;
                svuint64_t next_egg_word;
                // SPARSE: ~99% of word pairs have b3|b4==0 at 5% adult density
                if (__builtin_expect(
                    svptest_any(pg, svcmpne_n_u64(pg, svorr_x(pg, vb3, vb4), 0)),
                    false)) {
                    ...
                    store_hot_result_fast_le7(pg, vb0, vb1, vb2,
                        svld1_u64(pg, adults    + aligned_flat_word_index0),
                        svld1_u64(pg, juveniles + aligned_flat_word_index0),
                        svld1_u64(pg, eggs      + aligned_flat_word_index0),
                        next_adult_word, next_egg_word);
                }
                svstnt1_u64(pg, next_adults_ptr + aligned_flat_word_index0, next_adult_word);
                svstnt1_u64(pg, next_eggs_ptr + aligned_flat_word_index0, next_egg_word);
                ...
                svstnt1_u64(pg, next_adults_ptr + aligned_flat_word_index1, next_adult_word);
                svstnt1_u64(pg, next_eggs_ptr + aligned_flat_word_index1, next_egg_word);
    ...
    for (; row_index + 1 < hot_row_end; ++row_index) {
        ...
        if (use_aligned_main_hot_loop) {
            ...
            for (; word_index < aligned_simd_word_end; word_index += sve_words) {
                ...
                svstnt1_u64(pg, next_adults_ptr + aligned_flat_word_index, next_adult_word);
                svstnt1_u64(pg, next_eggs_ptr + aligned_flat_word_index, next_egg_word);
    ...
    if (use_aligned_main_hot_loop) {
        ...
        for (; word_index < simd_word_end; word_index += sve_words) {
            ...
            store_hot_result_from_total_with_center_u64_sve(
                ...
                next_adult_word,
                next_egg_word);
            svstnt1_u64(pg, next_adults_ptr + flat_word_index, next_adult_word);
            svstnt1_u64(pg, next_eggs_ptr + flat_word_index, next_egg_word);
        }
    }
