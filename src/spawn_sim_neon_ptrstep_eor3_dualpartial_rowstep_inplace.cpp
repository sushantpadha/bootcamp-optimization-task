// spawn_sim_neon_ptrstep_eor3_dualpartial_rowstep_inplace.cpp
//
// Builds on spawn_sim_neon_ptrstep_eor3_dualpartial_rowstep.cpp with one
// additional structural optimization:
//
//   inplace_adults: A(t+1) is written directly into the current_juveniles
//                  buffer instead of a separate next_adults buffer.
//                  Eliminates one full-grid write stream per generation and
//                  reduces the steady-state buffer count from 5 to 4.
//
//                  Safety: the hot kernel overwrites only interior words
//                  (words 1..words_per_row-2) of interior rows (rows 2..N-3).
//                  The wrapping and edge fallback passes read old juvenile bits
//                  only from the top/bottom 2 rows and from the first/last word
//                  of interior rows — none of which are touched by the hot path.
//                  Edge-word ORs target only the valid-mask bits, preserving
//                  the invalid columns (0-1 on the left, 62-63 on the right)
//                  that the wrapping fallback still needs to read.
//
//                  Generation rotation becomes a 3-swap cycle over 4 buffers:
//                    current_adults.swap(current_juveniles)  // A <- overwritten J
//                    current_juveniles.swap(current_eggs)    // J <- old E
//                    current_eggs.swap(next_eggs)            // E <- new eggs
//
// All four earlier optimizations (ptrstep, eor3, dualpartial, rowstep) are
// retained unchanged.
...
// ---------------------------------------------------------------------------
// rowstep + inplace_adults: emit next state into cur_juveniles (in-place),
// then subtract the outgoing partial and add the incoming partial.
//
// cur_juveniles is read (old J) then immediately overwritten with A(t+1).
// This is safe because juvenile_word is copied by value before the store.
// ---------------------------------------------------------------------------
...
__attribute__((always_inline)) static inline void emit_and_slide_neon(
    ...
    uint64_t* __restrict cur_juveniles,   // read J(t), write A(t+1) in-place
    ...
    uint64x2_t in_b0,  uint64x2_t in_b1,  uint64x2_t in_b2)
{
    ...
    // Write A(t+1) back into the juvenile buffer (in-place adults)
    vst1q_u64(cur_juveniles, vbslq_u64(adult_word, adult_5_to_10, juvenile_word));
...
// ---------------------------------------------------------------------------
// ptrstep + dualpartial + rowstep + inplace_adults hot helper
// ---------------------------------------------------------------------------

__attribute__((noinline)) static void run_two_hot_rows_pair2_neon_ptrstep(
    int pair_count,
    const uint64_t* __restrict row0_adults,
    uint64_t* __restrict row0_juveniles,    // read J(t), write A(t+1) in-place
    const uint64_t* __restrict row0_eggs,
    const uint64_t* __restrict row1_adults,
    uint64_t* __restrict row1_juveniles,    // read J(t), write A(t+1) in-place
...
__attribute__((noinline)) static void run_one_generation_hot_rows(
    ...
    BitPlane& current_juveniles,    // mutable: read J(t), overwritten with A(t+1)
    ...
    HotRowScratch& hot_row_scratch)
{
    ...
    const uint64_t* __restrict adults       = current_adults.data();
    uint64_t* __restrict juveniles_ptr      = current_juveniles.data();
        ...
        if (use_neon) {
            // -- Scalar preamble (word 1) ------------------------------------
            {
                ...
                store_hot_result_from_total_with_center(
                    b0, b1, b2, b3, b4,
                    adults[fi0], juveniles_ptr[fi0], eggs[fi0],
                    juveniles_ptr[fi0], next_eggs_ptr[fi0]);
                ...
                store_hot_result_from_total_with_center(
                    b0, b1, b2, b3, b4,
                    adults[fi1], juveniles_ptr[fi1], eggs[fi1],
                    juveniles_ptr[fi1], next_eggs_ptr[fi1]);
                subtract_horizontal_partial_from_total(
        ...
        if (use_neon) {
            {
                ...
                store_hot_result_from_total_with_center(
                    b0, b1, b2, b3, b4,
                    adults[fi], juveniles_ptr[fi], eggs[fi],
                    juveniles_ptr[fi], next_eggs_ptr[fi]);
                subtract_horizontal_partial_from_total(
            ...
            for (; word_index < aligned_neon_end; word_index += neon_words) {
                ...
                store_hot_result_from_total_with_center_u64_neon(
                    ...
                    vld1q_u64(juveniles_ptr + afi),
                    vld1q_u64(eggs          + afi),
                    next_adult, next_egg);
                vst1q_u64(juveniles_ptr + afi, next_adult);
...
// ---------------------------------------------------------------------------
// initialize_next_generation_rows — only clears next_eggs (no next_adults)
// ---------------------------------------------------------------------------
    ...
    BitPlane& next_eggs)
{
    ...
    for (int row_index = row_begin; row_index < row_end; ++row_index) {
        ...
        if (row_index < hot_row_begin || row_index >= hot_row_end) {
            ...
            next_eggs_ptr[row_base] = 0ULL;
            next_eggs_ptr[row_base + last_word_offset] = 0ULL;
        }
    }
}

// ---------------------------------------------------------------------------
// Edge handlers — write into current_juveniles instead of next_adults.
//
// Safety: the valid_mask used to gate adult_valid ensures that the OR only
// touches the valid interior bits of the edge word, leaving the invalid
// boundary columns (bits 0-1 on the left, bits 62-63 on the right)
// untouched so the wrapping fallback can still read J(t) there.
// ---------------------------------------------------------------------------
...
__attribute__((noinline)) static void run_one_generation_left_edge_valid_rows(
    ...
    const BitPlane& current_eggs,
    BitPlane& inplace_juveniles, BitPlane& next_eggs)
{
    const uint64_t* __restrict adults      = current_adults.data();
    const uint64_t* __restrict juveniles   = current_juveniles.data();
    const uint64_t* __restrict eggs        = current_eggs.data();
    uint64_t* __restrict juveniles_ptr     = inplace_juveniles.data();
    uint64_t* __restrict next_eggs_ptr     = next_eggs.data();
    ...
    for (int row_index = row_begin; row_index < row_end; ++row_index) {
        ...
        juveniles_ptr[r0] |= adult_valid & ge4 & ~ge10;
    ...
    BitPlane& inplace_juveniles, BitPlane& next_eggs)
{
    ...
    for (int row_index = row_begin; row_index < row_end; ++row_index) {
        ...
        juveniles_ptr[edge_wi] |= adult_valid & ge4 & ~ge10;
...
__attribute__((noinline)) static void run_one_generation_wrapping_rows(
    int row_begin, int row_end, int grid_size, int words_per_row,
    const BitPlane& current_adults,
    BitPlane& current_juveniles,    // read J(t) for block check, write A(t+1)
    const BitPlane& current_eggs,
    BitPlane& next_eggs)
{
    const uint64_t* __restrict adults       = current_adults.data();
    ...
    auto process_cell = [&](int row_index, int col_index) {
        ...
        if (((juveniles_ptr[fwi] | eggs[fwi]) & cell_mask) != 0) return;
        ...
        if ((adults[fwi] & cell_mask) != 0) {
            if (anc >= 4 && anc <= 9) juveniles_ptr[fwi] |= cell_mask;
        } else if (anc >= 3 && anc <= 5) {
    ...
    BitPlane& next_eggs)
{
    ...
    run_one_generation_wrapping_rows(
        row_begin, row_end, grid_size, words_per_row,
        current_adults, current_juveniles, current_eggs, next_eggs);
}
    ...
    HotRowScratch& hot_row_scratch)
{
    initialize_next_generation_rows(
        row_begin, row_end, grid_size, words_per_row, next_eggs);
...
int main(int argc, char* argv[])
{
    ...
    for (int generation = 0; generation < generations; ++generation) {
        ...
        process_generation_chunk(
            ...
            current_adults, current_juveniles, current_eggs,
            next_eggs, main_hot_row_scratch);

        generation_done_barrier.arrive_and_wait();

        // 4-buffer rotation: A <- overwritten J buffer, J <- old E, E <- new eggs
