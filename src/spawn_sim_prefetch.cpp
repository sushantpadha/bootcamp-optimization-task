    ...
    for (; row_index + 2 < hot_row_end; row_index += 2) {
        ...
        if (use_aligned_main_hot_loop) {
            ...
            for (; word_index < aligned_simd_word_end; word_index += sve_words) {
                ...
                // Prefetch juvs/eggs & incoming adult rows 32 words ahead (PLDL1STRM)
                __builtin_prefetch(juveniles       + aligned_flat_word_index0 + 32, 0, 0);
                __builtin_prefetch(eggs            + aligned_flat_word_index0 + 32, 0, 0);
                __builtin_prefetch(juveniles       + aligned_flat_word_index1 + 32, 0, 0);
                __builtin_prefetch(eggs            + aligned_flat_word_index1 + 32, 0, 0);
                __builtin_prefetch(incoming_adult_row0 + word_index + 32, 0, 0);
                __builtin_prefetch(incoming_adult_row1 + word_index + 32, 0, 0);
    ...
    for (; row_index + 1 < hot_row_end; ++row_index) {
        ...
        if (use_aligned_main_hot_loop) {
            ...
            for (; word_index < aligned_simd_word_end; word_index += sve_words) {
                ...
                const size_t partial_pair_index =
                    horizontal_partial_pair_index(word_index);
                // Prefetch juvs/eggs & incoming adult row 32 words ahead (PLDL1STRM)
                __builtin_prefetch(juveniles      + aligned_flat_word_index + 32, 0, 0);
                __builtin_prefetch(eggs           + aligned_flat_word_index + 32, 0, 0);
                __builtin_prefetch(incoming_adult_row + word_index + 32, 0, 0);
    ...
    if (use_aligned_main_hot_loop) {
        ...
        for (; word_index < aligned_simd_word_end; word_index += sve_words) {
            const size_t aligned_flat_word_index =
                ...
                // Prefetch adults/juvs/eggs 32 words ahead in final-row pass (PLDL1STRM)
                __builtin_prefetch(adults    + aligned_flat_word_index + 32, 0, 0);
                __builtin_prefetch(juveniles + aligned_flat_word_index + 32, 0, 0);
                __builtin_prefetch(eggs      + aligned_flat_word_index + 32, 0, 0);
            svuint64_t next_adult_word;
            svuint64_t next_egg_word;
