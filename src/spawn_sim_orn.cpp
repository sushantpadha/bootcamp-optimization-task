...
// SVE ORN: a | ~b.  No svorn_x intrinsic in GCC 14; use inline asm.
__attribute__((always_inline)) static inline svuint64_t
svorn_u64(svuint64_t a, svuint64_t b) {
    svuint64_t r;
    asm("orn %[r].d, %[a].d, %[b].d" : [r]"=w"(r) : [a]"w"(a), [b]"w"(b));
    return r;
}
...
{
    // BIC  = a & ~b  (svbic_x)
    // ORN  = a | ~b  (svorn_u64 via inline asm) — eliminates not_b1/not_b2 precompute
    // EOR3 = a^b^c   (sveor3)
    ...
    svuint64_t next_borrow = svbsl_u64(
        svorn_u64(partial_b1, total_b1),                       // p1 | ~b1 (ORN, 1 op)
        svbic_x(pg, partial_b1, total_b1),                     // p1 & ~b1
        borrow);
    total_b1 = sveor3(total_b1, partial_b1, borrow);
    borrow = next_borrow;

    next_borrow = svbsl_u64(
        svorn_u64(partial_b2, total_b2),                       // p2 | ~b2
        svbic_x(pg, partial_b2, total_b2),
        borrow);
    ...
    svuint64_t& next_egg_word)
{
    ...
    // BIC(egg_inner, ORR(b3,b4)): ORR(b3,b4) computed at t=5 (parallel to C/D),
    // reduces from 2 sequential BIC levels to 1  (-1 op, -1 cycle critical path)
