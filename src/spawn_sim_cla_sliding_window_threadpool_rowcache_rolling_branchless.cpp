...
// horizontal partial storage, branchless subtract updates, and an explicit
// SVE2 fast path for both horizontal partial generation and the main
// rolling-update hot loop for the Monster Spawning Grid reference simulator.
//
// INPUT FORMAT:
...
inline void add_horizontal_partial_to_total_u64_sve(
    ...
    svuint64_t& total_b4)
{
    const svuint64_t g0 = svand_x(pg, total_b0, partial_b0);
    const svuint64_t p0 = sveor_x(pg, total_b0, partial_b0);

    const svuint64_t g1 = svand_x(pg, total_b1, partial_b1);
    const svuint64_t p1 = sveor_x(pg, total_b1, partial_b1);

    const svuint64_t g2 = svand_x(pg, total_b2, partial_b2);
    const svuint64_t p2 = sveor_x(pg, total_b2, partial_b2);

    const svuint64_t c1 = g0;
    const svuint64_t c2 = svorr_x(pg, g1, svand_x(pg, p1, g0));
    const svuint64_t c3 = svorr_x(pg,
        svorr_x(pg, g2, svand_x(pg, p2, g1)),
        svand_x(pg, svand_x(pg, p2, p1), g0));
    const svuint64_t c4 = svand_x(pg, total_b3, c3);

    total_b0 = p0;
    total_b1 = sveor_x(pg, p1, c1);
    total_b2 = sveor_x(pg, p2, c2);
    total_b3 = sveor_x(pg, total_b3, c3);
    total_b4 = sveor_x(pg, total_b4, c4);
}
    ...
    svuint64_t& total_b4)
{
    ...
    const svuint64_t not_total_b0 = sveor_x(pg, total_b0, all_ones);
    const svuint64_t not_total_b1 = sveor_x(pg, total_b1, all_ones);
    const svuint64_t not_total_b2 = sveor_x(pg, total_b2, all_ones);

    const svuint64_t bg0 = svand_x(pg, not_total_b0, partial_b0);

    const svuint64_t bg1 = svand_x(pg, not_total_b1, partial_b1);
    const svuint64_t bp1 = svorr_x(pg, not_total_b1, partial_b1);

    const svuint64_t bg2 = svand_x(pg, not_total_b2, partial_b2);
    const svuint64_t bp2 = svorr_x(pg, not_total_b2, partial_b2);

    const svuint64_t borrow1 = bg0;
    const svuint64_t borrow2 = svorr_x(pg, bg1, svand_x(pg, bp1, bg0));
    const svuint64_t borrow3 = svorr_x(pg,
        svorr_x(pg, bg2, svand_x(pg, bp2, bg1)),
        svand_x(pg, svand_x(pg, bp2, bp1), bg0));
    const svuint64_t borrow4 = svand_x(pg,
        sveor_x(pg, total_b3, all_ones),
        borrow3);

    total_b0 = sveor_x(pg, total_b0, partial_b0);
    total_b1 = sveor_x(pg, sveor_x(pg, total_b1, partial_b1), borrow1);
    total_b2 = sveor_x(pg, sveor_x(pg, total_b2, partial_b2), borrow2);
    total_b3 = sveor_x(pg, total_b3, borrow3);
    total_b4 = sveor_x(pg, total_b4, borrow4);
}
...
inline void add_horizontal_partial_to_total(
    ...
    uint64_t& total_b4)
{
    // Generate and propagate from original values — all independent
    const uint64_t g0 = total_b0 & partial_b0;
    const uint64_t p0 = total_b0 ^ partial_b0;

    const uint64_t g1 = total_b1 & partial_b1;
    const uint64_t p1 = total_b1 ^ partial_b1;

    const uint64_t g2 = total_b2 & partial_b2;
    const uint64_t p2 = total_b2 ^ partial_b2;

    // Carries computed in parallel — no chain between c1, c2, c3
    const uint64_t c1 = g0;
    const uint64_t c2 = g1 | (p1 & g0);
    const uint64_t c3 = g2 | (p2 & g1) | (p2 & p1 & g0);
    const uint64_t c4 = total_b3 & c3;   // partial_b3 = 0

    total_b0 = p0;
    total_b1 = p1 ^ c1;
    total_b2 = p2 ^ c2;
    total_b3 = total_b3 ^ c3;
    total_b4 = total_b4 ^ c4;
}
...
inline void subtract_horizontal_partial_from_total(
    ...
    uint64_t& total_b4)
{
    // Borrow generate and propagate from original values
    const uint64_t bg0 = ~total_b0 & partial_b0;

    const uint64_t bg1 = ~total_b1 & partial_b1;
    const uint64_t bp1 = ~total_b1 | partial_b1;

    const uint64_t bg2 = ~total_b2 & partial_b2;
    const uint64_t bp2 = ~total_b2 | partial_b2;

    // Borrows computed in parallel — no chain between borrow1, borrow2, borrow3
    const uint64_t borrow1 = bg0;
    const uint64_t borrow2 = bg1 | (bp1 & bg0);
    const uint64_t borrow3 = bg2 | (bp2 & bg1) | (bp2 & bp1 & bg0);
    const uint64_t borrow4 = ~total_b3 & borrow3;   // partial_b3 = 0

    total_b0 = total_b0 ^ partial_b0;
    total_b1 = total_b1 ^ partial_b1 ^ borrow1;
    total_b2 = total_b2 ^ partial_b2 ^ borrow2;
    total_b3 = total_b3 ^ borrow3;
    total_b4 = total_b4 ^ borrow4;
}
    ...
    HorizontalPartialRow& row_partial)
{
    ...
    for (; word_index < simd_word_end; word_index += sve_words) {
        ...
        const svuint64_t partial_b1 = sveor_x(pg, carry_1, carry_2);
        const svuint64_t partial_b2 = svand_x(pg, carry_1, carry_2);
        const svuint64x3_t partial_triplet =
            svcreate3_u64(sum_2, partial_b1, partial_b2);
        svst3_u64(
            pg,
    ...
    for (; word_index < hot_word_end; ++word_index) {
        ...
        HorizontalPartialWord& partial_word =
            partial_words[static_cast<size_t>(word_index)];
        partial_word.b0 = sum_2;
        partial_word.b1 = carry_1 ^ carry_2;
        partial_word.b2 = carry_1 & carry_2;
    }
}
    ...
    VerticalAccumulatorRow& rolling_total)
{
    ...
    for (int word_index = hot_word_begin; word_index < hot_word_end; ++word_index) {
        ...
        uint64_t b3 = 0ULL;
        uint64_t b4 = 0ULL;
        const size_t partial_word_index = static_cast<size_t>(word_index);

        for (const HorizontalPartialRow& row_partial : row_partials) {
            const HorizontalPartialWord& partial_word =
                row_partial.words[partial_word_index];
    ...
    HotRowScratch& hot_row_scratch)
{
    ...
    for (int row_index = hot_row_begin; row_index + 1 < hot_row_end; ++row_index) {
        ...
        for (; word_index < simd_word_end; word_index += sve_words) {
            ...
            const svuint64x3_t outgoing_partials =
                svld3_u64(
                    pg,
                    reinterpret_cast<const uint64_t*>(outgoing_partial_words + word_index));
            const svuint64x3_t incoming_partials =
                svld3_u64(
                    pg,
                    reinterpret_cast<const uint64_t*>(incoming_partial_words + word_index));

            svuint64_t b0 = svld1_u64(pg, total_b0_ptr + word_index);
            svuint64_t b1 = svld1_u64(pg, total_b1_ptr + word_index);
            svuint64_t b2 = svld1_u64(pg, total_b2_ptr + word_index);
            svuint64_t b3 = svld1_u64(pg, total_b3_ptr + word_index);
            svuint64_t b4 = svld1_u64(pg, total_b4_ptr + word_index);
            ...
            svst1_u64(pg, total_b0_ptr + word_index, b0);
            svst1_u64(pg, total_b1_ptr + word_index, b1);
            svst1_u64(pg, total_b2_ptr + word_index, b2);
            svst1_u64(pg, total_b3_ptr + word_index, b3);
            svst1_u64(pg, total_b4_ptr + word_index, b4);
        }

        for (; word_index < hot_word_end; ++word_index) {
            const size_t flat_word_index = row_base + word_index;
            const HorizontalPartialWord& outgoing_partial =
                outgoing_partial_words[word_index];
            const HorizontalPartialWord& incoming_partial =
                incoming_partial_words[word_index];
