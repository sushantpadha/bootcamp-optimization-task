    ...
    HotRowScratch& hot_row_scratch)
{
    ...
    for (int row_index = hot_row_begin; row_index + 1 < hot_row_end; ++row_index) {
        ...
        for (; word_index < simd_word_end; word_index += sve_words) {
            ...
            // Prefetch the next row's read data at this column position.
            // The prefetch is issued one full row of processing time before
            // the data is needed, hiding DRAM latency across the row boundary.
            __builtin_prefetch(adults    + flat_word_index + words_per_row, 0, 1);
            __builtin_prefetch(juveniles + flat_word_index + words_per_row, 0, 1);
            __builtin_prefetch(eggs      + flat_word_index + words_per_row, 0, 1);
        ...
        for (; word_index < hot_word_end; ++word_index) {
            ...
            // Same prefetch for the scalar tail.
            __builtin_prefetch(adults    + flat_word_index + words_per_row, 0, 1);
            __builtin_prefetch(juveniles + flat_word_index + words_per_row, 0, 1);
            __builtin_prefetch(eggs      + flat_word_index + words_per_row, 0, 1);
