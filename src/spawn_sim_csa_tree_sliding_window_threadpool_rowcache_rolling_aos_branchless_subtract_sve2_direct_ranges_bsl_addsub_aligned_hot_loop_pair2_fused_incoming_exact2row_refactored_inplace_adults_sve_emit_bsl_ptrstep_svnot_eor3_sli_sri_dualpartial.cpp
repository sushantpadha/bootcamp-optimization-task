...
struct HotRowKernelContext {
    ...
    uint64_t* total_b0 = nullptr;
    uint64_t* total_b1 = nullptr;
    uint64_t* total_b2 = nullptr;
    uint64_t* total_b3 = nullptr;
    uint64_t* total_b4 = nullptr;
...
public:
    ...
    {
        context.total_b0[word_index] = counts.b0;
        context.total_b1[word_index] = counts.b1;
        context.total_b2[word_index] = counts.b2;
        context.total_b3[word_index] = counts.b3;
        context.total_b4[word_index] = counts.b4;
    }
...
public:
    ...
    {
        count_b0 = load_word(context.total_b0 + word_index);
        count_b1 = load_word(context.total_b1 + word_index);
        count_b2 = load_word(context.total_b2 + word_index);
        count_b3 = load_word(context.total_b3 + word_index);
        count_b4 = load_word(context.total_b4 + word_index);
    }
    ...
    ALWAYS_INLINE void store_counts(
        ...
        svuint64_t count_b4) const
    {
        store_word(context.total_b0 + word_index, count_b0);
        store_word(context.total_b1 + word_index, count_b1);
        store_word(context.total_b2 + word_index, count_b2);
        store_word(context.total_b3 + word_index, count_b3);
        store_word(context.total_b4 + word_index, count_b4);
    }

    ALWAYS_INLINE void load_counts_from_ptrs(
        const uint64_t* total_b0_ptr,
        const uint64_t* total_b1_ptr,
        const uint64_t* total_b2_ptr,
        const uint64_t* total_b3_ptr,
        const uint64_t* total_b4_ptr,
        svuint64_t& count_b0,
        svuint64_t& count_b1,
        svuint64_t& count_b2,
        svuint64_t& count_b3,
        svuint64_t& count_b4) const
    {
        count_b0 = load_word(total_b0_ptr);
        count_b1 = load_word(total_b1_ptr);
        count_b2 = load_word(total_b2_ptr);
        count_b3 = load_word(total_b3_ptr);
        count_b4 = load_word(total_b4_ptr);
    }

    ALWAYS_INLINE void store_counts_to_ptrs(
        uint64_t* total_b0_ptr,
        uint64_t* total_b1_ptr,
        uint64_t* total_b2_ptr,
        uint64_t* total_b3_ptr,
        uint64_t* total_b4_ptr,
        svuint64_t count_b0,
        svuint64_t count_b1,
        svuint64_t count_b2,
        svuint64_t count_b3,
        svuint64_t count_b4) const
    {
        store_word(total_b0_ptr, count_b0);
        store_word(total_b1_ptr, count_b1);
        store_word(total_b2_ptr, count_b2);
        store_word(total_b3_ptr, count_b3);
        store_word(total_b4_ptr, count_b4);
    }
...
struct VerticalAccumulatorRow {
    // Backing arrays for the rolling exact-count accumulator of the currently
    // active hot row. Index `word_index` across these five planes to recover the
    // full 5-bit adult-neighbour count for that output word.
    BitPlane b0;
    BitPlane b1;
    BitPlane b2;
    BitPlane b3;
    ...
    // Allocates zero-initialized storage for the five exact-count planes of one row.
    ...
    VerticalAccumulatorRow& rolling_total)
{
    ...
    for (int word_index = hot_word_begin; word_index < hot_word_end; ++word_index) {
        ...
        rolling_total.b0[word_index] = rolling_counts.b0;
        rolling_total.b1[word_index] = rolling_counts.b1;
        rolling_total.b2[word_index] = rolling_counts.b2;
        rolling_total.b3[word_index] = rolling_counts.b3;
        rolling_total.b4[word_index] = rolling_counts.b4;
    }
}
...
__attribute__((noinline)) static void run_two_hot_rows_pair2_sve_ptrstep(
    ...
    uint64_t* __restrict total_b0_ptr,
    uint64_t* __restrict total_b1_ptr,
    uint64_t* __restrict total_b2_ptr,
    uint64_t* __restrict total_b3_ptr,
    uint64_t* __restrict total_b4_ptr,
    ...
    HorizontalPartialPair2* partial_pairs1)
{
    ...
    for (int pair_index = 0; pair_index < pair_count; ++pair_index) {
        ...
        svuint64_t count_b3;
        svuint64_t count_b4;
        vector_backend.load_counts_from_ptrs(
            total_b0_ptr,
            total_b1_ptr,
            total_b2_ptr,
            total_b3_ptr,
        ...
        vector_backend.emit_next_state_from_word_ptrs(
            row0_adults_ptr,
            row0_juveniles_ptr,
            row0_eggs_ptr,
            row0_next_eggs_ptr,
        ...
        vector_backend.store_counts_to_ptrs(
            total_b0_ptr,
            total_b1_ptr,
            total_b2_ptr,
            total_b3_ptr,
    ...
    HotRowScratch& hot_row_scratch)
{
    ...
    HotRowKernelContext context{
        ...
        .total_b0 = hot_row_scratch.rolling_total.b0.data(),
        .total_b1 = hot_row_scratch.rolling_total.b1.data(),
        .total_b2 = hot_row_scratch.rolling_total.b2.data(),
        .total_b3 = hot_row_scratch.rolling_total.b3.data(),
        .total_b4 = hot_row_scratch.rolling_total.b4.data(),
...
int main(int argc, char* argv[])
{
    ...
    std::barrier bar(thread_count, [&]() noexcept {
