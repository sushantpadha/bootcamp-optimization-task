    ...
    HotRowScratch& hot_row_scratch)
{
    ...
    for (; row_index + 2 < hot_row_end; row_index += 2) {
        ...
        if (use_aligned_main_hot_loop) {
            ...
            // Sub-pass A: row0 OUTPUT — reads T (total), reads DRAM adults/juvs/eggs,
            // writes next_adults/next_eggs for row0.
            for (int ww = hot_word_begin + 1; ww < aligned_simd_word_end; ww += sve_words) {
                const size_t f0 = row0_base + static_cast<size_t>(ww);
                svuint64_t vb0 = svld1_u64(pg, total_b0_ptr + ww);
                svuint64_t vb1 = svld1_u64(pg, total_b1_ptr + ww);
                svuint64_t vb2 = svld1_u64(pg, total_b2_ptr + ww);
                svuint64_t vb3 = svld1_u64(pg, total_b3_ptr + ww);
                svuint64_t vb4 = svld1_u64(pg, total_b4_ptr + ww);
                svuint64_t next_adult_word, next_egg_word;
                store_hot_result_from_total_with_center_u64_sve(
                    pg, vb0, vb1, vb2, vb3, vb4,
                    svld1_u64(pg, adults + f0),
                    svld1_u64(pg, juveniles + f0),
                    svld1_u64(pg, eggs + f0),
                    next_adult_word, next_egg_word);
                svst1_u64(pg, next_adults_ptr + f0, next_adult_word);
                svst1_u64(pg, next_eggs_ptr + f0, next_egg_word);
            }

            // Sub-pass B: row0 UPDATE — reads T, applies subtract+add carry chains,
            // writes T' back; stores new incoming partial for row0.
            for (int ww = hot_word_begin + 1; ww < aligned_simd_word_end; ww += sve_words) {
                const size_t pi = horizontal_partial_pair_index(ww);
                svuint64_t vb0 = svld1_u64(pg, total_b0_ptr + ww);
                svuint64_t vb1 = svld1_u64(pg, total_b1_ptr + ww);
                svuint64_t vb2 = svld1_u64(pg, total_b2_ptr + ww);
                svuint64_t vb3 = svld1_u64(pg, total_b3_ptr + ww);
                svuint64_t vb4 = svld1_u64(pg, total_b4_ptr + ww);
                svuint64_t out_b0, out_b1, out_b2, in_b0, in_b1, in_b2;
                load_horizontal_partial_pair2_u64_sve(pg, outgoing_partial_pairs0[pi], out_b0, out_b1, out_b2);
                compute_horizontal_partial_pair2_u64_sve_from_adult_row(pg, incoming_adult_row0, ww, in_b0, in_b1, in_b2);
                subtract_horizontal_partial_from_total_u64_sve(pg, out_b0, out_b1, out_b2, vb0, vb1, vb2, vb3, vb4);
                add_horizontal_partial_to_total_u64_sve(pg, in_b0, in_b1, in_b2, vb0, vb1, vb2, vb3, vb4);
                svst1_u64(pg, total_b0_ptr + ww, vb0);
                svst1_u64(pg, total_b1_ptr + ww, vb1);
                svst1_u64(pg, total_b2_ptr + ww, vb2);
                svst1_u64(pg, total_b3_ptr + ww, vb3);
                svst1_u64(pg, total_b4_ptr + ww, vb4);
                store_horizontal_partial_pair2_u64_sve(pg, recycled_partial_pairs0[pi], in_b0, in_b1, in_b2);
            }

            // Sub-pass C: row1 OUTPUT — reads T' (updated by sub-pass B), reads DRAM
            // adults/juvs/eggs, writes next_adults/next_eggs for row1.
            for (int ww = hot_word_begin + 1; ww < aligned_simd_word_end; ww += sve_words) {
                const size_t f1 = row1_base + static_cast<size_t>(ww);
                svuint64_t vb0 = svld1_u64(pg, total_b0_ptr + ww);
                svuint64_t vb1 = svld1_u64(pg, total_b1_ptr + ww);
                svuint64_t vb2 = svld1_u64(pg, total_b2_ptr + ww);
                svuint64_t vb3 = svld1_u64(pg, total_b3_ptr + ww);
                ...
                store_hot_result_from_total_with_center_u64_sve(
                    pg, vb0, vb1, vb2, vb3, vb4,
                    svld1_u64(pg, adults + f1),
                    svld1_u64(pg, juveniles + f1),
                    svld1_u64(pg, eggs + f1),
                    next_adult_word, next_egg_word);
                svst1_u64(pg, next_adults_ptr + f1, next_adult_word);
                svst1_u64(pg, next_eggs_ptr + f1, next_egg_word);
            }

            // Sub-pass D: row1 UPDATE — reads T', applies subtract+add carry chains,
            // writes T'' back; stores new incoming partial for row1.
            for (int ww = hot_word_begin + 1; ww < aligned_simd_word_end; ww += sve_words) {
                const size_t pi = horizontal_partial_pair_index(ww);
                svuint64_t vb0 = svld1_u64(pg, total_b0_ptr + ww);
                svuint64_t vb1 = svld1_u64(pg, total_b1_ptr + ww);
                ...
                svuint64_t vb4 = svld1_u64(pg, total_b4_ptr + ww);
                svuint64_t out_b0, out_b1, out_b2, in_b0, in_b1, in_b2;
                load_horizontal_partial_pair2_u64_sve(pg, outgoing_partial_pairs1[pi], out_b0, out_b1, out_b2);
                compute_horizontal_partial_pair2_u64_sve_from_adult_row(pg, incoming_adult_row1, ww, in_b0, in_b1, in_b2);
                subtract_horizontal_partial_from_total_u64_sve(pg, out_b0, out_b1, out_b2, vb0, vb1, vb2, vb3, vb4);
                add_horizontal_partial_to_total_u64_sve(pg, in_b0, in_b1, in_b2, vb0, vb1, vb2, vb3, vb4);
                svst1_u64(pg, total_b0_ptr + ww, vb0);
                svst1_u64(pg, total_b1_ptr + ww, vb1);
                ...
                svst1_u64(pg, total_b3_ptr + ww, vb3);
                svst1_u64(pg, total_b4_ptr + ww, vb4);
                store_horizontal_partial_pair2_u64_sve(pg, recycled_partial_pairs1[pi], in_b0, in_b1, in_b2);
            }

            // Advance word_index past the SVE region (for trailing scalar loop below).
    ...
    for (; row_index + 1 < hot_row_end; ++row_index) {
        ...
        if (use_aligned_main_hot_loop) {
            ...
            // Sub-pass 1 (1-row): OUTPUT — reads T, reads DRAM adults/juvs/eggs,
            // writes next_adults/next_eggs.
            for (int ww = hot_word_begin + 1; ww < aligned_simd_word_end; ww += sve_words) {
                const size_t f = row_base + static_cast<size_t>(ww);
                svuint64_t vb0 = svld1_u64(pg, total_b0_ptr + ww);
                svuint64_t vb1 = svld1_u64(pg, total_b1_ptr + ww);
                svuint64_t vb2 = svld1_u64(pg, total_b2_ptr + ww);
                svuint64_t vb3 = svld1_u64(pg, total_b3_ptr + ww);
                ...
                store_hot_result_from_total_with_center_u64_sve(
                    pg, vb0, vb1, vb2, vb3, vb4,
                    svld1_u64(pg, adults + f),
                    svld1_u64(pg, juveniles + f),
                    svld1_u64(pg, eggs + f),
                    next_adult_word, next_egg_word);
                svst1_u64(pg, next_adults_ptr + f, next_adult_word);
                svst1_u64(pg, next_eggs_ptr + f, next_egg_word);
            }

            // Sub-pass 2 (1-row): UPDATE — reads T, applies subtract+add carry chains,
            // writes T' back; stores new incoming partial.
            for (int ww = hot_word_begin + 1; ww < aligned_simd_word_end; ww += sve_words) {
                const size_t pi = horizontal_partial_pair_index(ww);
                svuint64_t vb0 = svld1_u64(pg, total_b0_ptr + ww);
                svuint64_t vb1 = svld1_u64(pg, total_b1_ptr + ww);
                ...
                svuint64_t vb4 = svld1_u64(pg, total_b4_ptr + ww);
                svuint64_t out_b0, out_b1, out_b2, in_b0, in_b1, in_b2;
                load_horizontal_partial_pair2_u64_sve(pg, outgoing_partial_pairs[pi], out_b0, out_b1, out_b2);
                compute_horizontal_partial_pair2_u64_sve_from_adult_row(pg, incoming_adult_row, ww, in_b0, in_b1, in_b2);
                subtract_horizontal_partial_from_total_u64_sve(pg, out_b0, out_b1, out_b2, vb0, vb1, vb2, vb3, vb4);
                add_horizontal_partial_to_total_u64_sve(pg, in_b0, in_b1, in_b2, vb0, vb1, vb2, vb3, vb4);
                svst1_u64(pg, total_b0_ptr + ww, vb0);
                svst1_u64(pg, total_b1_ptr + ww, vb1);
                ...
                svst1_u64(pg, total_b3_ptr + ww, vb3);
                svst1_u64(pg, total_b4_ptr + ww, vb4);
                store_horizontal_partial_pair2_u64_sve(pg, recycled_partial_pairs[pi], in_b0, in_b1, in_b2);
            }
