    ...
    svuint64_t& next_adult_word, svuint64_t& next_egg_word)
{
    ...
    const svuint64_t egg_cond   = svorr_x(pg,
        svbic_x(pg, svand_x(pg, vb1, vb0), vb2),
        svbic_x(pg, vb2, vb1));
    const svuint64_t empty = svnot_x(pg, svorr_x(pg, adult_word,
        svorr_x(pg, juvenile_word, egg_word)));
    ...
    for (; row_index + 2 < hot_row_end; row_index += 2) {
        ...
        if (use_aligned_main_hot_loop) {
            ...
            for (; word_index < aligned_simd_word_end; word_index += sve_words) {
                ...
                svuint64_t next_adult_word;
                svuint64_t next_egg_word;
                const svuint64_t _b34 = svorr_x(pg, vb3, vb4);
                const svuint64_t _a0 = svld1_u64(pg, adults    + aligned_flat_word_index0);
                const svuint64_t _j0 = svld1_u64(pg, juveniles + aligned_flat_word_index0);
                const svuint64_t _e0 = svld1_u64(pg, eggs      + aligned_flat_word_index0);
                if (__builtin_expect(svptest_any(pg, svcmpne_n_u64(pg, _b34, 0)), false)) {
                    store_hot_result_from_total_with_center_u64_sve(
                        pg, vb0, vb1, vb2, vb3, vb4, _a0, _j0, _e0,
                        next_adult_word, next_egg_word);
                } else {
                    store_hot_result_fast_le7(pg, vb0, vb1, vb2, _a0, _j0, _e0,
                        next_adult_word, next_egg_word);
                }
                ...
                {
                    const svuint64_t _b34u = svorr_x(pg, vb3, vb4);
                    const svuint64_t _a1 = svld1_u64(pg, adults    + aligned_flat_word_index1);
                    const svuint64_t _j1 = svld1_u64(pg, juveniles + aligned_flat_word_index1);
                    const svuint64_t _e1 = svld1_u64(pg, eggs      + aligned_flat_word_index1);
                    if (__builtin_expect(svptest_any(pg, svcmpne_n_u64(pg, _b34u, 0)), false)) {
                        store_hot_result_from_total_with_center_u64_sve(
                            pg, vb0, vb1, vb2, vb3, vb4, _a1, _j1, _e1,
                            next_adult_word, next_egg_word);
                    } else {
                        store_hot_result_fast_le7(pg, vb0, vb1, vb2, _a1, _j1, _e1,
                            next_adult_word, next_egg_word);
                    }
                }
