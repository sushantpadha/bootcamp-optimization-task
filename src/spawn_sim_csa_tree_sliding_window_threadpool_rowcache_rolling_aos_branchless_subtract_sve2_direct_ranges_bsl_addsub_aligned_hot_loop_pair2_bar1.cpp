...
static constexpr int TARGET_THREAD_COUNT = 7;
    ...
    HotRowScratch& hot_row_scratch)
{
    ...
    for (int row_index = hot_row_begin; row_index + 1 < hot_row_end; ++row_index) {
        ...
        const HorizontalPartialPair2* __restrict outgoing_partial_pairs =
            outgoing_row.pairs.data();
        const HorizontalPartialPair2* __restrict incoming_partial_pairs =
            hot_row_scratch.incoming_partial.pairs.data();
        ...
        if (use_aligned_main_hot_loop) {
            ...
            const HorizontalPartialWord outgoing_partial =
                load_horizontal_partial_scalar(outgoing_row, word_index);
            const HorizontalPartialWord incoming_partial =
                load_horizontal_partial_scalar(hot_row_scratch.incoming_partial, word_index);
...
int main(int argc, char* argv[])
{
    ...
    auto worker_body = [&](int worker_id) {
        ...
        for (int generation = 0; generation < generations; ++generation) {
            process_generation_chunk(
                owned_rows.begin, owned_rows.end,
    ...
    for (int generation = 0; generation < generations; ++generation) {
        process_generation_chunk(
            main_owned_rows.begin, main_owned_rows.end,
            grid_size, words_per_row,
            current_adults, current_juveniles, current_eggs,
            next_adults, next_eggs,
